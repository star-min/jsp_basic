...
insert into board values((select nvl(max(seq), 0)+1 from board), '지금은 마지막 시간입니다.', '하지만 이걸 보고있는 당신은 다를수도 있습니다.', '관리자', sysdate, 0);
insert into board values((select nvl(max(seq), 0)+1 from board), '최강의 햄버거 프렌차이즈 투표', '결과는 1위 맘스터치, 2위 노브랜드, 3위 버거킹, 4위 맥도날드, 꼴찌 롯데리아   투표는 내맘속에서 했고 님들 롯데리아갈바엔 편의점햄버거드세요', '관리자', sysdate, 0);
insert into board values((select nvl(max(seq), 0)+1 from board), '난 이 게시판에 갇혀있어! 꺼내줘!', '랄까나?', '관리자', sysdate, 0);
insert into board values((select nvl(max(seq), 0)+1 from board), '제가 즐겨보는 스트리머가 있는데', '사실 우왁굳보다 고멤이 더 재미있습니다 왁굳형 감 많이 떨어진것같아요 차가리만 준내하구 고멤합방이나 많이해줘 형 그리고 리뷰할때 제발 스킵좀하지마 멀미날것같아', '관리자', sysdate, 0);
--테이블 board 검색
select * from board;
