...
insert into board values((select nvl(max(seq), 0)+1 from board), '이봐! 난지금 이 게시판에 갇혀있어!! 꺼내줘!', '랄까나?', '농담두', sysdate, 0);
insert into board values((select nvl(max(seq), 0)+1 from board), '제가 즐겨보는 게임스트리머가 있는데 우왁굳이라고 아세요?', '사실 우왁굳보다 고멤이 더 재미있습니다 왁굳형 감 많이 떨어진것같아요 차가리만 준내하구 고멤합방이나 많이해줘 형 그리고 리뷰할때 제발 스킵좀하지마 멀미날것같아', '시청자', sysdate, 0);
--테이블 board 검색
select * from board;
...
insert into human values ('academy', '1111', '학원생', '010-1111-1111', 'academy@hello.com', '1998-02-30', sysdate);
