...
<header id="header" class="header">
   ...
   <ul class="list_style01">
	<li><span class="bold"> 개발자 브랜든 아이크, Ecma International</span>
		<p>
	...
	<h3 class="h3_tit">역사</h3>
   <ul class="list_style01">
   <li><span class="bold"><a href="https://www.nextree.co.kr/p7323/"> JavaScript는 클래스라는 개념이 없습니다. 그래서 기존의 객체를 복사하여(cloning)<br> 
	새로운 객체를 생성하는 프로토타입 기반의 언어입니다. 프로토타입 기반 언어는 객체 원형인 프로토타입을 이용하여 새로운 객체를 만들어냅니다.<br>
	이렇게 생성된 객체 역시 또 다른 객체의 원형이 될 수 있습니다. 프로토타입은 객체를 확장하고 객체 지향적인 프로그래밍을 할 수 있게 해줍니다.[</a></span>
		<p>
		부모 자식의 구분도 없으며, 메인 메소드 같은 것도 없고, 클래스-인스턴스 관계도 없으며, 모든 함수가 클래스도 될 수 있으며 인스턴스도 될 수 있다.<br> 
		그렇기 때문에 모든 함수가 따로 놀기에 따라서 객체 지향처럼 "서로 맞물리는" 것을 기대할 수도 없다.(그런 이유로 클로저 개념이 있다.) <br>
		이 특성을 잘 이해해야 한다. 오히려 순서대로 읽는 절차 지향/객체 지향 언어보다는 모든 요소가 각자 따로 움직이는 <br>
		스타크래프트 맵 에디터 스크립트에 더 가까운 개념으로 이해해야 한다. 로드는 순서대로지만 실행은 동시 실행이다. <br>
		그러니까 main()같은건 없고 필요할때 실행되고 사라지는 스크립트들의 집합이라고 보면 된다. <br>
		오히려 웹 페이지 자체가 main()이라고 봐야 할 지경.
		</p><br><br><br>
		<p>
		JavaScript는 C에서 영향을 받은 C-Family 언어로 기본적인 문법이 유사 중괄호로 <br>
		구분하는 블럭, 세미콜론으로 줄이 끝남을 알리는 것, 변수 쓰는 법, 연산자 사용법 등 기초적인 문법이 C 문법과 크게 다르지 않다. <br>
		호이스팅 같은 것도 C언어의 잔재이다. 무엇보다 다른언어 쓰다온 사람들을 힘들게 하는건 클래스와 메소드가 외관으로 구분이 안된다라는 점이다.
		</p>
		<br><br><br>
		<p>
		JavaScript에서도 함수형 언어의 불편함과 호이스팅 등 여러 문제를 무마하기 위해서 클래스 개념의 도입과 <br>
		변수 선언을 let, const 로 하는 등 다른 언어 쓰는 사람들의 적응을 돕도록 흉내는 내 주는 척하는게 다행.<br> 
		물론 일반적인 객체 지향 프로그래밍 개발자에게는 이것마저 이해를 못 하고 Syntatic Sugar에 불과한 JavaScript의 클래스를 불만족스러워한다.
		</p>
