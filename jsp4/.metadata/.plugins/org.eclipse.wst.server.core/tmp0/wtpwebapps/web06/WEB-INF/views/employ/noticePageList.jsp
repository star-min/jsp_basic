...
<div id="contents" class="contents">
	<div class="container px-4 px-lg-5 mt-5">
		<div class="row">
			...
			<div class="row">
				<a href="addSmartEmployForm.do" class="btn btn-primary" style="display:inline-block;">글쓰기</a>
			</div>
			</c:if>
				...
				<table class="table responsive" id="myTable">
					...
					<c:choose>
						<c:when test="${cnt gt 0 }">	
							<c:forEach items="${employPageList}" var="notice" varStatus="status">
							<tr>
								<td>${employ.seq }</td>
								<td>${employ.title }</td>
								<td>${employ.regdate }</td>
							</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
			...
			<div class="row">
				...
				function list(page){
					location.href="../employ/pageList.do?curPage="+page;
				}
				</script>
			</div>
		</div>
