...
<header id="header" class="header">
    ...
    <div class="contents" id="conts">
		<div id="ct_text">
			...
			<p class="mb15"><a href="https://whichjdk.com" class="counting">https://whichjdk.com</a>에서 자세한 사항(영문)을 볼 수 있다.</p>
			<ol class="style_decimal">
				<li><a class="counting">오라클</a> '$''T': 릴리즈된 최신 버전만 일반 지원 일자 동안 공개 제공, 그 외는 오라클 서비스 구독 고객에게 비공개 제공</li>
				<li><a class="counting">OpenJDK</a> 'C''N': 릴리즈된 최신 버전만 제공, 그 외에는 소스만 제공. </li>
				<li><a class="counting">Azul</a> 'C''T': 6 이후 모든 버전 제공, 기업 사용 무료이나 유료 기술지원 제공 </li>
				<li><a class="counting">AdoptOpenJDK</a> 'C''N': LTS 8, 11 버전 및 최신 버전 제공, 17 이후는 아래 Adoptium 에서 이관하여 Temurin 이라는 배포 명칭으로 제공</li>
				<li><a class="counting">JBRT</a>: JetBrains 에서 내부적으로 JDK 사용하여 IDE 개발 후, 런타임만 IDE에 포함하여 제공. 공개 제공하지 않는다.</li>
				<li><a class="counting">Microsoft JDK</a> 'C''T''N': LTS 11, 17 제공, 주로 자사 Azure 클라우드 환경을 위한 JDK로 제공</li>
				<li><a class="counting">BellSoft Liberica JDK</a> 'C''T': Spring 에서 공식 런타임으로 지정한 JDK 로 상기 Zulu 처럼 상업적으로도 무료로 사용 가능하며 기술지원을 유료로 제공한다.</li>
				<li><a class="counting">IBM Semeru Runtime</a> 'T': IBM에서 개발한 JDK로 IBM 클라우드 전용으로 제공한다.</li>
				<li><a class="counting">Amazon Corretto</a> 'C''T': 오라클의 라이선스 정책 변경으로 태어난, AWS 에서 사용하기 적합한 JDK로, <br>
				다른 클라우드와 마찬가지로 클라우드의 JDK 사용 시 라이선스 저촉이 우려되어 이렇게 대형 클라우드사에서 JDK를 배포하고 있다.</li>
				<li><a class="counting">Alibaba Dragonwell</a> 'T': 알리바바가 만든 JDK로, 알리바바는 생각보다 OpenJDK 기여율이 높은 것으로 알려져 있다.<br> 
				그렇다 보니 중국에서는 이 JDK 를 통한 자바 개발을 독려 중. 그런 이유가 아니라면 선택지가 많으므로 <br>
				중국 내 자바 개발 외에는 쓰지 않는 것을 추천한다.</li>
				<li><a class="counting">SapMachine</a> 'T': SAP 에서 만든 JDK로, 완전히 SAP 전용이다. 따라서 SAP 개발이 아니라면 볼 일이 없다.</li>
				<li><a class="counting">Red Hat OpenJDK</a> 'T': 레드햇에서 만든 JDK로, 레드햇 리눅스 및 그 기반인 CentOS 에서도 <br>
				...
				<li><a class="counting">GraalVMCT</a> '$''N': 현재 오라클 에서 적극적으로 밀고 있는 JDK로 자바 뿐만 아니라 CLR 같은 방식으로 <br>
			...
			<div class="wrap_freeline">
				<dl class="private_info">
					...
					<dd>
					<li><a class="counting">Clojure</a></li>
					<li><a class="counting">Groovy</a>: Java에 Python, Ruby, SmallTalk 등의 특징을 버무린 동적 타입 언어. 2003년에 등장했다.<br>
					Gradle 패키지 관리의 기본 언어이다.</li>
					<li><a class="counting">JRuby</a>: Ruby의 대부분을 Java로 구현한 언어 구현체. 역시 Java와 호환된다.</li>
					<li><a class="counting">Jython</a>: 파이썬의 Java 구현체. JVM 위에서 동작하며, 따라서 Python 모듈과 함께 Java 클래스, JAR 사용이 가능하다. <br>
					참고로 로고는 Java의 커피 컨셉과 Python의 뱀 컨셉을 섞어서 커피잔에서 뱀이 연기처럼 피어나오는 로고다(...)</li>
					<li><a class="counting">Kotlin</a> : IntelliJ IDEA를 만든 JetBrains에서 개발한 JVM 언어. Google I/O 2017에서 안드로이드의 공식 언어로 지정되었다.[8]</li>
					<li><a class="counting">Rhino</a>: JVM으로 돌아가는 JavaScript 엔진으로, 모질라 재단이 100% Java로 개발했다. <br>
					JavaScript와 Java의 API를 동시에 사용해 개발할 수 있다. Java 7 에 내장되어 있다.</li>
					<li><a class="counting">Nashorn</a>: 또 다른 JVM 위에서 돌아가는 JavaScript 엔진이다. Java 8과 함께 등장했다. <br>
					그러나 Java 11 부터 제거 예정이며, Java 15에서 완전히 제거되었다. 참고로 Nashorn 은 Rhino(코뿔소)의 독일어다. <br>
					제거 이후는 GraalVM이 대신한다.</li>
					<li><a class="counting">Scala</a></li>
					</dd>
				</dl>
			</div>
		</div>
