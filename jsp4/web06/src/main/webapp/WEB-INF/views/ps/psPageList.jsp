...
<div id="contents" class="contents">
	<div class="container px-4 px-lg-5 mt-5">
		<div class="row">
			<br><br>		
			<h2 class="tit">언론보도 목록</h2>
			<c:if test="${member.id == 'admin' }">
			<div class="row">
				<a href="addSmartPsForm.do" class="btn btn-primary" style="display:inline-block;">글쓰기</a>
			</div>
			</c:if>
				...
				<table class="table responsive" id="myTable">
					...
					<c:choose>
						<c:when test="${cnt gt 0 }">	
							<c:forEach items="${psPageList}" var="ps" varStatus="status">
							<tr>
								<td>${ps.seq }</td>
								<td>${ps.title }</td>
								<td>${ps.regdate }</td>
